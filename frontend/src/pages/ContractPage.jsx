...
export default function ContractPreviewPage() {
  ...
  const [artistSignature, setArtistSignature] = useState(null); // from booking
  ...
  useEffect(() => {
    async function fetchBooking() {
      try {
        ...
        // Set user role based on logged in user vs booking artist/client
        if (user && data) {
    ...
    fetchBooking();
  }, [bookingId, user]); // Add user to dependency array
  ...
  const handleArtistSign = async () => {
    ...
    setLoading(true);
    try {
      await axios.post("/contracts/sign-artist", {
        bookingId,
  ...
  return (
    <div className="p-6 w-full max-w-4xl mx-auto text-sm">
      ...
      <section className="border p-8 rounded-lg bg-white text-gray-800 shadow-md leading-relaxed">
        ...
        <p>This Agreement is made on this <u>___</u> day of <u>____________</u>, 20<u>__</u>.</p>

        {/* Client & Artist Info */}
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h4 className="font-semibold underline">Client/Organizer:</h4>
            <p><span className="font-medium">Name:</span> {booking.client.username}</p>
          ...
          <div className="md:text-left">
            ...
            <p><span className="font-medium">Name:</span> {booking.artist.username}</p>
            <p><span className="font-medium">Phone:</span> {booking.artist.phone}</p>
            <p><span className="font-medium">Email:</span> {booking.artist.email}</p>
          </div>
        </div>
          ...
          <label className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (/^\d*$/.test(value)) setWage(value); 
              }}
              inputMode="numeric"
              className="border-b border-gray-400 px-1 w-24 focus:outline-none focus:border-blue-500 appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]"
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-col space-y-1 mt-2 max-w-xs">
            {["check", "cash", "bankTransfer", "other"].map((method) => (
              <label key={method} className="inline-flex items-center space-x-2">
                ...
                <span className="capitalize">
                  {method === "bankTransfer" ? "Bank Transfer" : method === "other" ? "Other:" : method.charAt(0).toUpperCase() + method.slice(1)}
                </span>
        ...
        <div className="mt-4">
          ...
          <p>The Client agrees to pay a non-refundable advance payment of 50% of the total wage (Rs. {Math.floor(wage / 2) || "_____"}) to confirm the booking, shall be paid on the day of the event prior to performance.</p>
        </div>
        ...
        <div className="mt-4">
          ...
          <p>Client agrees to provide a safe environment for the Artist. The Client assumes all responsibility for any injury or damage occurring at the event, except where caused by the Artist’s negligence.</p>
        </div>
